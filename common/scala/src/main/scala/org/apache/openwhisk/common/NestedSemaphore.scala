...
class NestedSemaphore[T](permits: Int) extends ForcibleSemaphore(permits) {
   ...
   *     - if resource slot acquired, release concurrency slots
   * - release the lock
   * - if neither concurrency slot nor resource slot acquired, return false
   * @param actionid
   * @param maxConcurrent
   * @param permits
   * @param force
  ...
  def releaseConcurrent(actionid: T, maxConcurrent: Int, permits: Int): Unit = {
    ...
    if (maxConcurrent == 1) {
      ...
      val (resourceRelease, actionRelease) = concurrentSlots.release(1, true)
      //concurrent slots
      if (resourceRelease) {
        super.release(permits)
      }
